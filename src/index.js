...
class ServerlessSharedApiGateway {
  ...
  createRestApi () {
    return this.provider.request('APIGateway', 'createRestApi', {
  ...
  async loadResourcesForApi () {
    ...
    this.resources = []
    do {
      const {position, items} = await this.provider.request('APIGateway', 'getResources', {position: currentPosition, restApiId: this.restApiId, limit: 500})
  ...
  async findRestApi () {
    const {items} = await this.provider.request('APIGateway', 'getRestApis', {})
